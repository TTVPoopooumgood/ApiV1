const userID = "clanChampionship"; // Change this to your Discord user ID
...
const elements = {
	...
	clanId: document.querySelector(".clanId"),
...
async function fetchDiscordStatus() {
	try {
		const [lanyardResponse, lookupResponse] = await Promise.all([
			fetch(`https://api.kirka.io/api/leaderboard/${userID}`).then((response) =>
				response.json()
			),
			fetch(`https://api.kirka.io/api/leaderboard/${userID}`).then(
